...
async function createWindow() {
  ...
  win.on("close", (e) => {
    if (!confirmQuitDialog()) e.preventDefault();
  });
      ...
      },
      {
        ...
        click() {
          const file = dialog.showOpenDialogSync(win, {
            ...
            filters: [{ name: "Файл проекта", extensions: ["vproj"] }],
            properties: ["openFile"],
          });
          var content = "";
          if (file) {
            ...
            win.webContents.send("dispatchAction", {
              type: ModelActions.SET_PROJECT,
              payload: content,
            });
          }
        },
      ...
      },
      {
        ...
        click() {
          win.webContents.send(
            "dispatchAction",
            ScreenActions.print({ save: false })
          );
        },
      },
      {
...
function confirmQuitDialog(){
  return confirmDialog("Выйти из программы?")
}